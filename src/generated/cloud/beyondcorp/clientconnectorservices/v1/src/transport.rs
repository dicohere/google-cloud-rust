...
impl super::stub::ClientConnectorServicesService for ClientConnectorServicesService {
    async fn list_client_connector_services(
        ...
        let path =
            format!("/v1/{}/clientConnectorServices",
                    {
                        let arg = &req.parent;
                        if arg.is_empty() {
    ...
    async fn create_client_connector_service(
        ...
        self.inner.execute(
            builder,
            Some(req.client_connector_service),
            options,
        ).await
    }
    ...
    async fn update_client_connector_service(
        ...
        let path =
            format!("/v1/{}",
                    {
                        let arg = &req.client_connector_service.as_ref().ok_or_else(|| gaxi::path_parameter::missing("client_connector_service"))?.name;
                        if arg.is_empty() {
                            return Err(gaxi::path_parameter::missing("client_connector_service.name"));
                        }
                        arg
                    },
            );
