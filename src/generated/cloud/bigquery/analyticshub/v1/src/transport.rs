...
impl super::stub::AnalyticsHubService for AnalyticsHubService {
    ...
    async fn update_data_exchange(
        ...
        let path =
            format!("/v1/{}",
                    {
                        let arg = &req.data_exchange.as_ref().ok_or_else(|| gaxi::path_parameter::missing("data_exchange"))?.name;
                        if arg.is_empty() {
    ...
    async fn update_listing(
        ...
        let path =
            format!("/v1/{}",
                    {
                        let arg = &req.listing.as_ref().ok_or_else(|| gaxi::path_parameter::missing("listing"))?.name;
                        if arg.is_empty() {
    ...
    async fn list_shared_resource_subscriptions(
        ...
        let builder = self
            ...
            .query(&[("$alt", "json;enum-encoding=int")])
            .header("x-goog-api-client", reqwest::header::HeaderValue::from_static(&crate::info::X_GOOG_API_CLIENT_HEADER));
        let builder = builder.query(&[("includeDeletedSubscriptions", &req.include_deleted_subscriptions)]);
        let builder = builder.query(&[("pageSize", &req.page_size)]);
        let builder = builder.query(&[("pageToken", &req.page_token)]);
        self.inner.execute(
            builder,
