...
impl super::stub::KeyTrackingService for KeyTrackingService {
    async fn get_protected_resources_summary(
        ...
        let path =
            format!("/v1/{}/protectedResourcesSummary",
                    {
                        let arg = &req.name;
                        if arg.is_empty() {
    ...
    async fn search_protected_resources(
        ...
        let path =
            format!("/v1/{}/protectedResources:search",
                    {
                        let arg = &req.scope;
                        if arg.is_empty() {
        ...
        let builder = builder.query(&[("pageSize", &req.page_size)]);
        let builder = builder.query(&[("pageToken", &req.page_token)]);
        let builder = req.resource_types.iter().fold(builder, |builder, p| builder.query(&[("resourceTypes", p)]));
        self.inner.execute(
            builder,
            None::<gaxi::http::NoBody>,
            options,
