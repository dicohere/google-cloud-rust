...
impl super::stub::StorageTransferService for StorageTransferService {
    async fn get_google_service_account(
        ...
        let path =
            format!("/v1/googleServiceAccounts/{}",
                    {
    ...
    async fn update_agent_pool(
        ...
        let path =
            format!("/v1/{}",
                    {
                        let arg = &req.agent_pool.as_ref().ok_or_else(|| gaxi::path_parameter::missing("agent_pool"))?.name;
                        if arg.is_empty() {
